...
lock = RLock()
idFilosofo = 2 # 1,2,3,4 ou 5 #if do filosofo precisa corresponder ao indice no array de ips de seu respectivo ip
ipsFilosofosReal = list(["200.239.138.232", "200.239.138.237","200.239.138.92","200.239.138.205","200.239.138.234"])
ipsFilosofos = list(["200.239.138.232", "0.0.0.0","200.239.138.92","200.239.138.205","200.239.138.234"])
...
class Filosofo(Thread):
    ...
    def __init__(self,cond):
        ...
        self.op = cond
    def comer(self):
        global idFilosofo,ipsFilosofos,ipsFilosofosReal,portaInicio,portaJanta,situacaoFilosofo,meuGarfo,garfoAmigo,comecou
        print("comer")
        situacaoFilosofo = 1
        time.sleep(random.random())  # comendo por tempo indeterminado
        situacaoFilosofo= 0
        for i in ipsFilosofos:
            array = ["Filosofo ",ipsFilosofosReal[idFilosofo - 1]," comendo com garfos ",str(meuGarfo)," e ",str(garfoAmigo)]
    ...
    def pegarGarfoAmigo(self):
        ...
        if(garfoAmigo != 0):
            return True
        self.enviaDado(ipsFilosofos[idFilosofo % len(ipsFilosofos)],
                  5060,str("1|-1"))  # 1 para pegar garfo
        print("pegarGarfoAmigo | msg enviada ao amigo,esperando...")
        try:
            msg, ipFilosofoAmigo = self.recebeDado(
                ipsFilosofos[idFilosofo - 1], 5061,1)
        ...
        self.enviaDado(ipsFilosofos[idFilosofo % len(ipsFilosofos)],
                  5060,str("ok"))
        print("Garfo Amigo " + str(garfoAmigo) + ",meu garfo " + str(meuGarfo) + " mensagem " + msg)
    ...
    def devolverGarfoAmigo(self):
        ...
        self.enviaDado(ipsFilosofos[idFilosofo % len(ipsFilosofos)], 5060, "".join(array))  # 2 para devolver
        try:
    ...
    def viver(self):
        ...
        while(True):
            self.dormir()
            tentativas += 1
            print("meuGarfo : " +str(meuGarfo) + "  garfoAmigo : "+str(garfoAmigo))
    ...
    def imprimeJantar(self):
        global idFilosofo,ipsFilosofos,ipsFilosofosReal,portaInicio,portaJanta,situacaoFilosofo,meuGarfo,garfoAmigo,comecou
        print("imprimeJantar")
        msg, ipFilosofo = self.recebeDado(ipsFilosofos[idFilosofo - 1], portaJanta,0)
        print(msg)
    ...
    def inicio(self):
        global idFilosofo,ipsFilosofos,ipsFilosofosReal,portaInicio,portaJanta,situacaoFilosofo,meuGarfo,garfoAmigo,comecou
        cond = int(raw_input("1 para ouvir e 2 para executar: "))
        if(cond == 1):
            msg, ipFilosofoAmigo = self.recebeDado(
            ...
            msg = raw_input("Digite inicio para comecar: ")
            for i in ipsFilosofos:
                print(i)
